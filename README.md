# plan-Universal - Интерактивная схема зданий с интеграцией Google Таблицы
...
Для настройки помещений, которые должны всегда отображаться синим цветом, используется конфигурационный файл `blue-rooms-config.json` в корневой директории проекта.
...
Где:
- `building` - идентификатор здания в формате "building-XX"
- `floor` - идентификатор этажа в формате "floor-Y" (0 для подвала)
- `rooms` - массив номеров помещений на указанном этаже указанного здания, которые должны быть отображены синим цветом

Если конфигурационный файл не найден или содержит ошибки, будет использована базовая конфигурация с несколькими помещениями.
...
2. `loadBlueRoomsConfig()` - загрузка конфигурации синих помещений из файла
3. `getDefaultBlueRooms()` - получение резервных данных о синих помещениях
4. `loadSVG(filename)` - загрузка SVG-схемы по имени файла
5. `updateRoomStatus()` - обновление цветов помещений на основе данных из таблицы и конфигурации
6. `shouldBeBlue(roomNumber, building, floor)` - проверка, должно ли помещение быть синим
7. `displayRoomInfo(roomId)` - отображение информации о помещении при клике
8. `fetchDataFromGoogleSheets()` - получение данных из Google Таблицы через Apps Script
9. `normalizeRoomNumber(str)` - нормализация номеров помещений для корректного сравнения
10. `translateLatinToCyrillic(text)` - конвертация латинских букв (a, b, c) в кириллические (а, б, в)
11. `updateSpecialRoomsHighlight()` - выделение особых помещений на схеме
...
- Приоритетное отображение синих помещений на основе конфигурационного файла
- Резервная конфигурация при отсутствии файла или ошибке загрузки
...
5. Для тестирования конфигурации синих помещений:
   - В консоли выполните команду `blueRooms` для просмотра текущей конфигурации
   - При необходимости временно измените конфигурацию в консоли:
     ```javascript
     blueRooms = [
       {
         building: "building-19",
     ...
     updateRoomStatus(); // Для применения изменений
     ```
   - Для просмотра базовой конфигурации используйте `getDefaultBlueRooms()`
